...
function getNoteKey(year: number, month: number, day: number) {
  return `attendance-note-${year}-${month}-${day}`;
}
...
export function CalendarView({
  ...
  const _firstDayOfWeek = new Date(year, month - 1, 1).getDay();
  ...
  const openDayDialog = (day: number) => {
    ...
    setNoteValue(localStorage.getItem(getNoteKey(year, month, day)) || "");
    setDialogOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      localStorage.setItem(getNoteKey(year, month, selectedDay), noteValue);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      localStorage.removeItem(getNoteKey(year, month, selectedDay));
  ...
  // Represent blanks as negative numbers for stable keys
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-3 mt-4 pt-3 border-t border-border">
        <div className="flex items-center gap-1.5">
          <span className="w-2.5 h-2.5 rounded-sm bg-working inline-block" />
          <span className="text-xs text-muted-foreground">Working</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="w-2.5 h-2.5 rounded-sm bg-leave inline-block" />
          <span className="text-xs text-muted-foreground">Leave</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="w-2.5 h-2.5 rounded-sm bg-tour inline-block" />
